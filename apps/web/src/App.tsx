...
function RegistrationForm({
  ...
  const submitPassword = async () => {
    ...
    try {
      ...
      await onParticipantSessionRefresh().catch(() => undefined);
...
function EventPage({
  ...
  const handleSignOut = async () => {
    ...
    try {
      ...
      setSignOutError("Не удалось безопасно завершить сеанс. Попробуйте ещё раз.");
    } finally {
    ...
    <PageFrame privacyDocument={privacyDocument} onSignIn={!authenticatedAccount ? openSignIn : undefined}>
      {loginOpener && !authenticatedAccount ? (
        <SignInDialog
          ...
          onAuthenticated={(account) => {
            setLoginOpener(null);
            handleAuthenticatedAccountChange(account);
          }}
        />
      ) : null}
