...
import {
  ...
  ADMIN_MEMBER_TRIBE_STATUS_OPTIONS,
...
function MemberDetailDrawer({
  ...
  const [profileDraft, setProfileDraft] = useState<AdminMemberProfileDraft>(() =>
    createAdminMemberProfileDraft(member, profile),
  );
  ...
  const saveProfile = useCallback(async () => {
    ...
    const fieldsResult = buildAdminMemberProfileUpdateFields(detail, profileDraft);
...
function MemberProfileSection({
  ...
  member: AdminMemberListRow;
  onCancelEdit: () => void;
  onDraftChange: (draft: AdminMemberProfileDraft) => void;
  onSaveEdit: () => void;
  onStartEdit: () => void;
  profile: AdminMemberProfile | null;
  saving: boolean;
...
function formatBirthTimeContextLabel(value: string | null | undefined): string {
  ...
  return (
    ADMIN_MEMBER_BIRTH_TIME_CONTEXT_OPTIONS.find(
...
function formatTribeStatusLabel(value: string | null | undefined): string {
  ...
  return (
    ADMIN_MEMBER_TRIBE_STATUS_OPTIONS.find((option) => option.value === value)
      ?.label ?? value
  );
}
...
function formatMaritalStatusLabel(value: string | null | undefined): string {
  ...
  return (
    ADMIN_MEMBER_MARITAL_STATUS_OPTIONS.find((option) => option.value === value)
      ?.label ?? value
  );
}
