-- Admin occurrence read state must use PostgreSQL time, not browser time.

drop function if exists public.admin_list_event_occurrences(uuid);

create function public.admin_list_event_occurrences(p_event_id uuid)
returns table (
  id uuid,
  event_id uuid,
  title text,
