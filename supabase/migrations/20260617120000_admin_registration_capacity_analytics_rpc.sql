-- Admin registration capacity analytics RPC.
-- Keeps capacity overview calculations server-side for the web admin.
...
begin
  ...
  multi_meal_guests as (
    ...
    from (
      select acr.registration_id
      from active_capacity_reservations acr
      group by acr.registration_id
      having count(distinct acr.capacity_unit_id) > 1
    ) grouped
  ),
  bucket_stat_rows as (
    select
      acr.capacity_unit_id,
      coalesce(sum(acr.seats_count), 0)::integer as occupied_seats,
      count(*)::integer as reservations_count
    from active_capacity_reservations acr
    group by acr.capacity_unit_id
  ),
  bucket_option_rows as (
    select
      acr.capacity_unit_id,
      acr.option_id,
      coalesce(acr.option_title_snapshot, 'Option') as option_title,
      count(distinct acr.registration_id)::integer as registrations_count,
      coalesce(sum(acr.quantity), 0)::integer as quantity,
      coalesce(sum(acr.seats_count), 0)::integer as seats_count
    from active_capacity_reservations acr
    group by
      acr.capacity_unit_id,
      acr.option_id,
      coalesce(acr.option_title_snapshot, 'Option')
  ),
