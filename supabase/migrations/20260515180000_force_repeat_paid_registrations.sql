-- Belt-and-suspenders follow-up to 20260515170000_allow_repeat_paid_registrations.sql.
--
-- Symptom observed in dev: after two paid-simulated registrations on the same
-- (event, occurrence), the database held a single event_registrations row with
-- seats_count merged, instead of two distinct rows. The fix has two parts:
--
--   1. Drop every unique constraint/index on event_registrations that pins
--      (event_id, user_id) — including any auto-named leftover from the
--      original 0001_init.sql `unique(event_id, user_id)` definition or from
--      the partial active indexes added in 20260515133000.
--   2. Re-assert public.register_for_paid_event_simulated as a pure INSERT —
--      no ON CONFLICT, no lookup of an existing row, no seats merge. Each
...
declare
  ...
  v_index record;
begin
  -- Drop UNIQUE *constraints* whose columns are exactly {event_id, user_id}
  -- or any superset that still pins the (event_id, user_id) pair.
  for v_constraint in
    select c.conname
  ...
  loop
    execute format(
      'alter table public.event_registrations drop constraint if exists %I',
      v_constraint.conname
    );
  end loop;

  -- Drop standalone UNIQUE *indexes* (not backing a constraint) that pin
  -- (event_id, user_id) — including the partial active indexes from
  -- 20260515133000_event_registration_occurrence_uniqueness.sql.
  for v_index in
    select i.relname
    from pg_index ix
    ...
    where n.nspname = 'public'
      ...
      and (
        select array_agg(att.attname::text order by att.attname::text)
        from unnest(ix.indkey::int[]) as k(attnum)
        join pg_attribute att
...
begin
  ...
  -- A fresh row every call. Never ON CONFLICT, never UPDATE of an existing row.
