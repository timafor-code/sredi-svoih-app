...
type PrayersSettingsContentProps = {
  error: string | null;
  hasSavedVisibleNusach: boolean;
  onNusachPress: (value: VisibleNusach) => void;
  savingNusach: VisibleNusach | null;
  selectedNusach: VisibleNusach;
  unsavedNusachCopy: string;
};
...
export function resolveVisibleNusachSelection(value: string | null | undefined) {
  const savedNusach = isVisibleNusach(value) ? value : null;

  return {
    hasSavedVisibleNusach: savedNusach !== null,
    selectedNusach: savedNusach ?? 'chabad',
  } as const;
}
...
export function saveLocalNusachSelection({
  ...
  setNusach: (nusach: VisibleNusach) => void;
  value: VisibleNusach;
}) {
  setNusach(value);
...
export default function PrayersSettingsScreen() {
  return appCapabilities.isGuestOnly
    ? <LocalPrayersSettingsScreen />
    : <AccountPrayersSettingsScreen />;
}

function LocalPrayersSettingsScreen() {
  const storedNusach = useSettingsStore((state) => state.nusach);
  const setNusach = useSettingsStore((state) => state.setNusach);
  ...
  const { hasSavedVisibleNusach, selectedNusach } = resolveVisibleNusachSelection(
    storedNusach,
  );

  const handleNusachPress = useCallback((value: VisibleNusach) => {
  ...
  return (
    <PrayersSettingsContent
      error={null}
      hasSavedVisibleNusach={hasSavedVisibleNusach}
      onNusachPress={handleNusachPress}
      savingNusach={null}
      selectedNusach={selectedNusach}
      unsavedNusachCopy="Показан Хабад по умолчанию. Нажмите вариант, чтобы сохранить нусах на этом устройстве."
    />
  );
}
...
function AccountPrayersSettingsScreen() {
  ...
  const savedNusach: VisibleNusach | null = isVisibleNusach(profile?.nusach)
    ? profile.nusach
    : null;
  const selectedNusach = savedNusach ?? 'chabad';
  ...
  return (
    <PrayersSettingsContent
      ...
      hasSavedVisibleNusach={savedNusach !== null}
      onNusachPress={(value) => {
        void handleNusachPress(value);
      }}
      savingNusach={savingNusach}
      selectedNusach={selectedNusach}
      unsavedNusachCopy="Показан Хабад по умолчанию. Нажмите вариант, чтобы сохранить нусах в профиле."
    />
  );
}
        ...
        <IOSGroup>
          {visibleNusachOptions.map((option, index) => (
            <ListRow
              ...
              onPress={savingNusach ? undefined : () => onNusachPress(option.value)}
