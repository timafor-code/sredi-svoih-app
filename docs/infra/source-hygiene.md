...
- separately audits the five privacy workflow/evidence tables that are excluded from environment promotion;
...
The reviewed Alembic head for this contract is `20260816184500`.
...
`privacy_financial_review_evidence` is environment-bound because its pseudonymous subject hash is bound to deployment secrets. It is therefore excluded from environment promotion just like the existing privacy evidence tables.
...
2. Resolve every active deletion lifecycle through the canonical privacy-erasure worker before cleanup.
3. Classify every detected high-confidence signature and any remaining uncertain graph relationship.
4. Create a fresh source backup before any cleanup apply.
5. Run cleanup dry-run and review aggregate results.
6. Perform no destructive apply until the project owner gives a separate explicit approval.
7. Re-run this read-only audit after cleanup.
8. Resume `docs/infra/data-promotion.md` only after the source is reviewed and promotion guards are satisfied.

The audit itself does not perform cleanup and does not authorize production promotion.
